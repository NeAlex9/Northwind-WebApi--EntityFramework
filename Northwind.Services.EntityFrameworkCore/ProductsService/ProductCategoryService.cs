    ...
    public class ProductCategoryService : IProductCategoryService
    {
        ...
        public async Task<(bool isSuccess, ProductCategory productCategory)> TryGetCategoryAsync(int categoryId)
        {
            var categoryDto = await this.context
                .Categories
                .AsNoTracking()
                .Where(categoryDto => categoryId == categoryDto.CategoryId)
                .FirstOrDefaultAsync();
        ...
        public async Task<int> CreateCategoryAsync(ProductCategory productCategory)
        {
            ...
            var dto = this.mapper.Map<CategoryDTO>(productCategory);
            await this.context
                .Categories
                .AddAsync(dto);
        ...
        public async IAsyncEnumerable<ProductCategory> GetCategoriesByNameAsync(ICollection<string> names)
        {
            ...
            await foreach (var dto in this.context
                               .Categories
                               .AsNoTracking()
                               .Where(dto => names.Any(name => name == dto.CategoryName))
                               .AsAsyncEnumerable())
            {
        ...
        private static void UpdateCategoryDto(CategoryDTO categoryDto, ProductCategory productCategory)
        {
            categoryDto.Description = productCategory.Description;
            categoryDto.CategoryName = productCategory.Name;
        }
    }
}
