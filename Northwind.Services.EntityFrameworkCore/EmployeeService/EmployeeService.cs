...
namespace Northwind.Services.EntityFrameworkCore.EmployeeService
{
    /// <summary>
    /// 
    /// </summary>
    public class EmployeeService : IEmployeeService
    {
        ...
        public async Task<(bool isSuccess, Employee employee)> TryGetEmployeeIdAsync(int employeeId)
        {
            var dto = await this.context
                .Employees
                .Where(dto => dto.EmployeeId == employeeId)
                .FirstOrDefaultAsync();
            if (dto is null)
            {
                return (false, null);
        ...
        private static void UpdateEmployeeDto(EmployeeDTO dto, Employee employee)
        {
